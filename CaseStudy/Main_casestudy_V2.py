...
# SPARQL query_1: What are the entities that are not reusable anymore?
query_1 = prepareQuery(
    """
    PREFIX ex: <http://www.semanticweb.org/arnak/ontologies/2022/7/DOR#>
    PREFIX bpo: <https://www.projekt-scope.de/ontologies/bpo/#>
    ...
    SELECT ?entity
    WHERE {
        ?entity ex:hasRemainingReuseCount "0"^^xsd:int .
    }
    """,
    initNs={"ex": ex, "bpo": bpo, "dicm": dicm}
)

# SPARQL query_1: How many entities are not reusable anymore?
query_2 = """
PREFIX ex: <http://www.semanticweb.org/arnak/ontologies/2022/7/DOR#>
PREFIX bpo: <https://www.projekt-scope.de/ontologies/bpo/#>
...
WHERE {
  ?entity ex:hasRemainingReuseCount ?count .
  FILTER (?count = !0)
}
"""
...
# SELECT DISTINCT ?entity ?Potential2 WHERE {
# ?entity ex:hasCircularPotential ?Potential1 .
# ?entity ex:hasFinalCircularPotential ?Potential2 .
# ?Potential2 ex:hasCircularRequirement ?individual .
#  ex:Legal rdfs:subClassOf ?individual .
# }
# """,
...
#query = """
#SELECT (COUNT(?rebar_ent) AS ?rebar_ent_count) (COUNT(?rebar) AS ?rebar_count)
#WHERE {
#  ?slab bpo:isComposedOf ?rebar.
 # ?rebar rdf:type bpo:Element.
  
  #?rebar_ent bpo:realisesObject ?rebar.
  #?rebar_ent rdf:type bpo:Entity.
  
  #FILTER EXISTS { ?slab rdf:type bpo:Assembly }
#}
#"""
...
#for row in results:
#    rebar_ent_count = row["rebar_ent_count"]
#    rebar_count = row["rebar_count"]
#    print(f"Number of rebar_ent_uri instances: {rebar_ent_count}")
#    print(f"Number of rebar_uri instances: {rebar_count}")
